...
*/		
		...
		if (*count==1) mcs_broadcast(buf_not_empty);
		write_buf();
		mcs_unlock(mutex);
	}
	exit(0);
...
*/		
		...
		printf("%d\n",d);
		fflush(stdout);
		if (*count==BSIZE-1) mcs_broadcast(buf_not_full);
		write_buf();
		mcs_unlock(mutex);
	}
}
